	...
	public void removeCourse() //method to delete all students from a specified programme
	{
	   ...
	   LinearNode<Student> currentStudent, nodeToDelete=null;
		do {
			...
			{	
			deletedStudent = studentList.remove(nodeToDelete.getElement()); // removing the memory address of that node
			}
			
			if (deletedStudent != null) {
				System.out.println(deletedStudent.getName() + " was deleted from the list"); // printing the students
																								// from the code that
																								// have been deleted
			}
